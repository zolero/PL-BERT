...
class Measure:
    ...
    def __init__(self):
        ...
        self.of_a_regex = re.compile(
            r"(-?\d+ -?\d+ *\/ *-? *\d+)|(-?\d+ *(?:½|⅓|⅔|¼|¾|⅕|⅖|⅗|⅘|⅙|⅚|⅐|⅛|⅜|⅝|⅞|⅑|⅒))"
        )
        # Regex to detect input to pass to digit convertor, including potential million/billion suffix
        self.value_regex = re.compile(
            r"(-?(?: |\d)*\.?\d+ *(?:thousand|million|billion|trillion|quadrillion|quintillion|sextillion|septillion|octillion|undecillion|tredecillion|quattuordecillion|quindecillion|sexdecillion|septendecillion|octodecillion|novemdecillion|vigintillion)?)"
        )
        ...
        self.prefixable_trans_dict = {
            ...
            "Bps": {"singular": "byte per second", "plural": "bytes per second"},
            "g": {"singular": "gram", "plural": "grams"},
            "gf": {"singular": "gram force", "plural": "grams force"},
            "W": {"singular": "watt", "plural": "watts"},
            "Wh": {"singular": "watt hour", "plural": "watt hours"},
            ...
            "Ah": {"singular": "amp hour", "plural": "amp hours"},
        ...
        self.prefixed_dict = {
            prefix
            + prefixed: {
                "singular": self.prefix_dict[prefix]
                + self.prefixable_trans_dict[prefixed]["singular"],
                "plural": self.prefix_dict[prefix]
                + self.prefixable_trans_dict[prefixed]["plural"],
            }
            for prefixed in self.prefixable_trans_dict
        ...
        self.custom_dict = {
            ...
            "pc": {"singular": "percent", "plural": "percent"},
            ...
            "kt": {"singular": "knot", "plural": "knots"},
            "dB": {"singular": "decibel", "plural": "decibels"},
            "AU": {"singular": "astronomical unit", "plural": "astronomical units"},
            "st": {
                "singular": "stone",
            ...
            "sq": {"singular": "square", "plural": "square"},
            "2": {"singular": "square", "plural": "square"},
            "²": {"singular": "square", "plural": "square"},
            "3": {"singular": "cubic", "plural": "cubic"},
            "³": {"singular": "cubic", "plural": "cubic"},
            ...
            "ch": {"singular": "chain", "plural": "chains"},
            ...
            "kph": {"singular": "kilometer per hour", "plural": "kilometers per hour"},
            "Da": {"singular": "dalton", "plural": "daltons"},
            "cwt": {"singular": "hundredweight", "plural": "hundredweight"},
            "Sv": {
                "singular": "sievert",
                "plural": "sieverts",
            },
            "C": {"singular": "celcius", "plural": "celcius"},  # Overrides Coulomb
            "degrees": {"singular": "degree", "plural": "degrees"},
            "degree": {"singular": "degree", "plural": "degrees"},
            "atm": {"singular": "atmosphere", "plural": "atmospheres"},
            "min": {"singular": "minute", "plural": "minutes"},
            "cd": {"singular": "candela", "plural": "candelas"},
            "ly": {"singular": "light year", "plural": "light years"},
            "kts": {"singular": "knot", "plural": "knots"},
            "mol": {"singular": "mole", "plural": "moles"},
        ...
        self.lower_prefixed_dict = {
            key.lower(): self.prefixed_dict[key] for key in self.prefixed_dict
        }
    ...
    def convert(self, token: str) -> str:
        ...
        if match:
            ...
            if self.of_a_regex.match(match.group(0)):
                ...
                result_list.append(
                    "of an" if token and token[0] in list("aeiou") else "of a"
                )
            ...
            if match:
                ...
                result_list.append(
                    self.decimal.convert(
                        self.filter_space_regex.sub("", match.group(1))
                    )
                )
                token = token[: match.span()[0]] + token[match.span()[1] :]
                ...
                # Otherwise it's true
                if abs(
                    float(self.letter_filter_regex.sub("", match.group(1)))
                ) != 1 or "." in match.group(1):
                    plural = True
        ...
        for split_token in token.split(" "):
            for i, token in enumerate(self.split_token(split_token)):
                ...
                if token in self.prefixed_dict:
                    result_list.append(
                        self.prefixed_dict[token][
                            "plural" if plural and not per else "singular"
                        ]
                    )
                elif token.lower() in self.lower_prefixed_dict:
                    result_list.append(
                        self.lower_prefixed_dict[token.lower()][
                            "plural" if plural and not per else "singular"
                        ]
                    )
                else:

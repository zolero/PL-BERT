...
class Time:
    ...
    def __init__(self):
        ...
        self.time_regex = re.compile(
            r"^(?P<hour>\d{1,2}) *((?::|.) *(?P<minute>\d{1,2}))? *(?P<suffix>[a-zA-Z\. ]*)$",
            flags=re.I,
        )
        # Regex to detect time in the form xx:yy:zz
        self.full_time_regex = re.compile(
            r"^(?:(?P<hour>\d{1,2}) *:)? *(?P<minute>\d{1,2})(?: *: *(?P<seconds>\d{1,2})(?: *. *(?P<milliseconds>\d{1,2}))?)? *(?P<suffix>[a-zA-Z\. ]*)$",
            flags=re.I,
        )
        # Regex to detect time in the form AM5 and PM7
        self.ampm_time_regex = re.compile(
            r"^(?P<suffix>[a-zA-Z\. ]*)(?P<hour>\d{1,2})", flags=re.I
        )
    ...
    def convert(self, token: str) -> str:
        ...
        if match:
            ...
            if milliseconds:
                ...
                result_list.append(
                    "millisecond" if int(milliseconds) == 1 else "milliseconds"
                )
            # 3.5 If suffix, prepend the suffix with padded spaces
            if suffix:
                result_list += [c for c in suffix.lower() if c not in (" ", ".")]

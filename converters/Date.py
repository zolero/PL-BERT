...
class Date:
    ...
    def __init__(self):
        ...
        self.day_regex = re.compile(
            r"^(?P<prefix>monday|tuesday|wednesday|thursday|friday|saturday|sunday|mon|tue|wed|thu|fri|sat|sun)\.?",
            flags=re.I,
        )

        # Regex to check for yyyy-mm-dd date
        self.dash_date_ymd_regex = re.compile(
            r"^(?P<year>\d{2,5}) *(?:-|\.|/) *(?P<month>\d{1,2}) *(?:-|\.|/) *(?P<day>\d{1,2})$",
            flags=re.I,
        )
        # Regex to check for mm-dd-yyyy date
        self.dash_date_mdy_regex = re.compile(
            r"^(?P<month>\d{1,2}) *(?:-|\.|/) *(?P<day>\d{1,2}) *(?:-|\.|/) *(?P<year>\d{2,5})$",
            flags=re.I,
        )

        # Regex to check for YYYY-Month-DD
        self.text_ymd_regex = re.compile(
            r"^(?P<year>\d{2,5}) *(?:-|\.|/) *(?P<month>january|february|march|april|may|june|july|august|september|october|november|december|sept|jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec) *(?:-|\.|/) *(?P<day>\d{1,2})$",
            flags=re.I,
        )
        # Regex to check for DD-Month-YYYY
        self.text_dmy_regex = re.compile(
            r"^(?P<day>\d{1,2}) *(?:-|\.|/) *(?P<month>january|february|march|april|may|june|july|august|september|october|november|december|sept|jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec) *(?:-|\.|/) *(?P<year>\d{2,5})$",
            flags=re.I,
        )
        # Regex to check for Month-DD-YYYY
        self.text_mdy_regex = re.compile(
            r"^(?P<month>january|february|march|april|may|june|july|august|september|october|november|december|sept|jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec) *(?:-|\.|/) *(?P<day>\d{1,2}) *(?:-|\.|/) *(?P<year>\d{2,5})$",
            flags=re.I,
        )

        # Regex to check for DD Month YYYY, Month YYYY, YYYY or YYYYs
        self.dmy_regex = re.compile(
            r"^(?:(?:(?P<day>\d{1,2}) +(of +)?)?(?P<month>january|february|march|april|may|june|july|august|september|october|november|december|sept|jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)\.? +)?(?P<year>\d{1,5})(?P<suffix>s?)\/?(?: *(?P<bcsuffix>[A-Z\.]+)?)$",
            flags=re.I,
        )
        # Regex to check for Month DD, YYYY
        self.mdy_regex = re.compile(
            r"^(?P<month>january|february|march|april|may|june|july|august|september|october|november|december|sept|jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)?\.? *(?P<day>\d{1,2})? +(?P<year>\d{1,5})(?P<suffix>s?)\/?(?: *(?P<bcsuffix>[A-Z\.]+)?)$",
            flags=re.I,
        )

        # Regex to check for DD Month
        self.dm_regex = re.compile(
            r"^(?P<day>\d{1,2}) +(of +)?(?P<month>january|february|march|april|may|june|july|august|september|october|november|december|sept|jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)\.?(?: *(?P<bcsuffix>[A-Z\.]+)?)$",
            flags=re.I,
        )
        # Regex to check for Month DD
        self.md_regex = re.compile(
            r"^(?P<month>january|february|march|april|may|june|july|august|september|october|november|december|sept|jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)\.? +(?P<day>\d{1,2})(?: *(?P<bcsuffix>[A-Z\.]+)?)$",
            flags=re.I,
        )
    ...
    def convert(self, token: str) -> str:
        ...
        if match:
            ...
            try:
                suffix = " ".join(
                    [c for c in match.group("bcsuffix").lower() if c not in (" ", ".")]
                )
        ...
        # 3 Match "MM-DD-YY(YY)", "YY(YY)-MM-DD", "DD-Month-YY(YY)", "YY(YY)-Month-DD" or "Month-DD-YY(YY)"
        match = (
            self.dash_date_mdy_regex.match(token)
            or self.dash_date_ymd_regex.match(token)
            or self.text_dmy_regex.match(token)
            or self.text_ymd_regex.match(token)
            or self.text_mdy_regex.match(token)
        )
        if match:
            ...
            try:
                ...
                if (
                    match.group(0).startswith(month)
                    and int(day) > 12
                    or prefix
                    and match.group(0).endswith(year)
                    and int(month) <= 12
                ):
                    dmy = False
